...
/**
 * Public signup offers Individual, Business and Community. Admin is NOT
 * offered — admins are provisioned separately (by an existing admin or the
 * dev seed). The database enforces the same rule: the profiles INSERT policy
 * and handle_new_user only accept these three roles, so self-selecting admin
 * is blocked server-side no matter what the form sends.
 */
const ROLES: Array<{ value: UserRole; label: string; icon: string; blurb: string }> = [
  ...
  { value: "community", label: "Community", icon: "apartment", blurb: "Society / RWA — post official notices" },
];
...
export function SignupPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (user) {
        // Session granted immediately (email confirmation is off) — go to the
        // experience for this account type.
        navigate(homePathForRole(user.role), { replace: true });
      } else {
        // Email confirmation is enabled on this Supabase project: the account
        // exists but can't sign in yet. Show the message, then land the user
        // on the login page with a notice so they don't loop back to signup.
        setInfo("Almost there! Check your inbox for a confirmation link, then sign in.");
        setTimeout(() => {
          navigate("/login", {
            replace: true,
            state: { notice: "Account created! Check your inbox for a confirmation link, then sign in." },
          });
        }, 1200);
      }
    } catch (err) {
      ...
      <form onSubmit={onSubmit} className="space-y-6" noValidate>
        ...
        <Field label="I am a…">
          ...
          <p className="text-label-sm font-label-sm text-on-surface-variant mt-2">
            Community accounts post official society notices. Admin access is never available at signup.
          </p>
        </Field>
